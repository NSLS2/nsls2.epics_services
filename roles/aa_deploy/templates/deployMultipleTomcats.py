...
def deployMultipleTomcats(destFolder):
    ...
    if tomcatHome is None:
        print(
            "We determine the source Tomcat distribution using the environment variable TOMCAT_HOME which does not seem to be set."
        )
        sys.exit(1)
    thisAppliance = os.getenv("ARCHAPPL_MYIDENTITY")
    if thisAppliance is None:
        print(
            "We determine the identity of this appliance using the environment variable ARCHAPPL_MYIDENTITY which does not seem to be set."
        )
        sys.exit(1)
    appliancesXML = os.getenv("ARCHAPPL_APPLIANCES")
    if appliancesXML is None:
        print(
            "We determine the location of the appliances.xml file using the environment variable ARCHAPPL_APPLIANCES which does not seem to be set."
        )
        sys.exit(1)
    ...
    print(
        ...
        tomcatHome,
        "\n\tto generate deployments for appliance",
        thisAppliance,
        "\n\tusing configuration info from",
        appliancesXML,
        "\n\tinto folder",
        destFolder,
    )
    ...
    serverStopStartPort = (
        serverdom.getElementsByTagName("Server").item(0).getAttribute("port")
    )
    ...
    print(
        "The stop/start ports for the new instance will being at ",
        newServerStopStartPort,
    )
    ...
    for appliance in appliances:
        ...
        retrievalUrl = (
            appliance.getElementsByTagName("retrieval_url").item(0).firstChild.data
        )
        ...
        for app in ["mgmt", "engine", "etl", "retrieval"]:
            ...
            newServerDom.getElementsByTagName("Server").item(0).setAttribute(
            ...
            for httplistenerNode in newServerDom.getElementsByTagName("Connector"):
                if (
                    ...
                    print(
                        ...
                        httplistenerNode.getAttribute("protocol"),
                        ". If you do need this connector, you should un-comment this.",
                    )
                    comment = httplistenerNode.ownerDocument.createComment(
                        httplistenerNode.toxml()
                    )
                    httplistenerNode.parentNode.replaceChild(comment, httplistenerNode)

            if not haveSetConnector:
