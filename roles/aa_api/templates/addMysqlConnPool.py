...
if not mysql_connection_string:
    print(
        'The environment variable MYSQL_CONNECTION_STRING is not defined. Please define using export MYSQL_CONNECTION_STRING="--user=archappl --password=archappl --database=archappl"'
    )
    sys.exit(1)
...
if tomcatHome is None:
    print(
        "We determine the location of context.xml using the environment variable TOMCAT_HOME which does not seem to be set."
    )
    sys.exit(1)
...
if "user" not in connpoolparams:
    print(
        'Cannot determine the user from ", mysql_connection_string, ". Please define like so "--user=archappl --password=archappl --database=archappl"'
    )
    sys.exit(1)
...
# Default to localhost if host not specified
...
connpool.setAttribute("driverClassName", "org.mariadb.jdbc.Driver")
connpool.setAttribute("url", "jdbc:mariadb://" + host + ":3306/" + db)
connpool.setAttribute("username", user)
connpool.setAttribute("password", pwd)
